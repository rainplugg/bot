      ...
      private static async void ClientMessage(object sender, MessageEventArgs e)
      {
         try {
            ...
            if (message.Text == "/start") {
               if (user == null)
                  Connect.Query("insert into `User` (id, message, sub, id_list, balance, id_invited, sale) values (" + message.Chat.Id + ", 'none', 'none', 0, 0, 0, 25);");
               ...
               if (user == null)
                  Connect.Query("insert into `User` (id, message, sub, id_list, balance, id_invited, sale) values (" + message.Chat.Id + ", 'none', 'none', 0, 0, " + message.Text.Split(' ')[1] + ", 25);");
      ...
      private static async void UpdateData(object sender, UpdateEventArgs e)
      {
         try {
            ...
            if (update.ChannelPost != null) {
               ...
               if (user.Id_invited != 0) {
                  int price = users.Find(x => x.Id == user.Id_invited).Balance + (999 / 100 * user.Sale);
