...
namespace Course
{
   internal class Program
   {
      ...
      readonly static long adminChat = -1001845529685;
      ...
      private static async void ClientMessage(object sender, MessageEventArgs e)
      {
         try {
            ...
            if (message.Text == "/start") {
               if (user == null)
                  Connect.Query("insert into `User` (id, message, sub, id_list, balance, id_invited, sale) values (" + message.Chat.Id + ", 'none', 'none', 0, 0, 0, 0);");
               ...
               if (user == null)
                  Connect.Query("insert into `User` (id, message, sub, id_list, balance, id_invited, sale) values (" + message.Chat.Id + ", 'none', 'none', 0, 0, " + message.Text.Split(' ')[1] + ", 5);");
               else if (user.Id_invited == 0 && user.Id.ToString() != message.Text.Split(' ')[1])
                  Connect.Query("update `User` set id_invited = " + message.Text.Split(' ')[1] + ", sale = 5 where id = " + message.Chat.Id + ";");
               if (user != null && user.Sub == "active")
                  await client.SendPhotoAsync(message.Chat.Id, "AgACAgIAAxkBAAIEUWSHJx77F35S9B6HN8i_7gABueVmfwACTMgxG1N3OEjQJw2iZI-9fgEAAwIAA3gAAy8E", "Привет *" + message.From.FirstName + "*, мы рады приветствовать вас в сообщесте \"*Самоучки*\"\n" +
                                                                     "У нас вы сможете найти большое количество обучающих курсов лучших онлайн школ образования - SkillBox. " +
                                                                     "Нетология, Яндекс.Практикум, Udemy, SkillFactory и многие другие схемы заработка.\n" +
               ...
               if (user.Message == "waitspecial") {
                  ...
                  await client.SendTextMessageAsync(message.Chat.Id, "✅ Сообщение отправлено специалистам, ожидайте ответа, он поступит в этот чат", replyMarkup: new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("На главную", "GoToMain") } }));
               }
            }
            else {
         ...
         try {
            ...
            if (data == "Help") {
               ...
               if (users.Find(x => x.Id == message.Chat.Id).Id_invited != 0)
                  price = 999 - (999 / 100 * users.Find(x => x.Id == message.Chat.Id).Sale);
      ...
      private static async void UpdateData(object sender, UpdateEventArgs e)
      {
         try {
            ...
            if (update.ChannelPost != null) {
               ...
               else if (update.ChannelPost.Chat.Id == adminChat) {
                  if (update.ChannelPost.Text == "/count") {
                     Connect.LoadUser(users);
                     await client.SendTextMessageAsync(adminChat, "✅ /count (Количество пользователей)\n\nКоличество пользователь в боте: " + users.Count + "\nКоличество рефералов: " + users.FindAll(x => x.Id_invited != 0).Count);
                  }
                  else if (update.ChannelPost.Text == "/updatedb") {
                     await client.SendTextMessageAsync(adminChat, "✅ /updatedb (Обновление базы курсов)\n\nДанный процесс может занять несколько минут, пожалуйста, подождите");
                     Sheets.GetDataBase();
                  }
                  else if (update.ChannelPost.Text == "/refer") {
                     string response = "✅ /refer (Список наиболее используемых реф. ссылок)\n\n";
                     Connect.LoadUser(users);
                     List<Refer> refers = new List<Refer>();
                     int count = 0;
                     for (int i = 0; i < users.Count; i++) {
                        count = 0;
                        count = users.FindAll(x => x.Id_invited == users[i].Id).Count;
                        if (count != 0)
                           refers.Add(new Refer(users[i].Id, count));
                     }
                     refers.Sort(delegate (Refer id, Refer count) {
                        return id.Count.CompareTo(count.Count);
                     });
                     refers.Reverse();
                     foreach(var refer in refers) {
                        response += "https://t.me/samouchki_bot?start=" + refer.Id + " (" + refer.Count + " исп.)\n";
                     }
                     await client.SendTextMessageAsync(adminChat, response, disableWebPagePreview: true);
                  }
                  else if (update.ChannelPost.Text.Contains("/setsale ")) {
                     try {
                        if (update.ChannelPost.Text.Split(' ').Length == 3) {
                           if (Convert.ToInt32(update.ChannelPost.Text.Split(' ')[2]) >= 0 && Convert.ToInt32(update.ChannelPost.Text.Split(' ')[2]) <= 100) {
                              Connect.LoadUser(users);
                              var user = users.Find(x => x.Id.ToString() == update.ChannelPost.Text.Split(' ')[1]);
                              if (user != null) {
                                 Connect.Query("update `User` set sale = " + update.ChannelPost.Text.Split(' ')[2] + " where id = " + user.Id + ";");
                                 await client.SendTextMessageAsync(adminChat, "✅ /setsale (Установка процента скидки пользователю)\n\nСкидка пользователю \"" + user.Id + "\" в размере " + update.ChannelPost.Text.Split(' ')[2] + "% успешно установлена");
                              }
                              else
                                 await client.SendTextMessageAsync(adminChat, "⛔ /setsale (Установка процента скидки пользователю)\n\nПользователь в базе данных бота не найден");
                           }
                           else
